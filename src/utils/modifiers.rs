...
pub fn gap(val: f32) -> impl StyleModifier { |s: &mut Style| s.flex.gap = Some(val); }

// --- Sizing & Spacing ---
pub fn w(val: f32) -> impl StyleModifier { |s: &mut Style| s.sizing.width = Some(val); }
pub fn h(val: f32) -> impl StyleModifier { |s: &mut Style| s.sizing.height = Some(val); }
pub fn w_full() -> impl StyleModifier { |s: &mut Style| s.sizing.width = Some(100.0); } // Logic depends on container
pub fn h_full() -> impl StyleModifier { |s: &mut Style| s.sizing.height = Some(100.0); }

pub fn p(val: impl IntoSpacing) -> impl StyleModifier { |s: &mut Style| s.padding = val.into_spacing(); }
pub fn m(val: impl IntoSpacing) -> impl StyleModifier { |s: &mut Style| s.margin = val.into_spacing(); }

pub fn rounded(val: f32) -> impl StyleModifier { |s: &mut Style| s.rounding = crate::utils::border::Rounding::all(val); }
