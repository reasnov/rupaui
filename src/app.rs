        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                if let (Some(root), Some(node)) = (&self.root, self.root_node) {
                    self.dispatch_drag(root.as_ref(), node, delta);
                }
            }
            ...
            WindowEvent::RedrawRequested => {
                if let (Some(root), Some(ref mut renderer)) = (&self.root, &mut self.renderer) {
                    self.taffy.clear();
                    let root_node = root.layout(&mut self.taffy, None);
                    self.root_node = Some(root_node);
                    let size = renderer.size;
                    self.taffy.compute_layout(root_node, Size { width: AvailableSpace::Definite(size.width as f32), height: AvailableSpace::Definite(size.height as f32) }).unwrap();
                    if let Ok((output, view, mut encoder)) = renderer.begin_frame() {
                        {
                            let mut _pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor { label: Some("Rupaui Pass"), color_attachments: &[Some(wgpu::RenderPassColorAttachment { view: &view, resolve_target: None, ops: wgpu::Operations { load: wgpu::LoadOp::Clear(wgpu::Color::BLACK), store: wgpu::StoreOp::Store } })], depth_stencil_attachment: None, timestamp_writes: None, occlusion_query_set: None });
                            root.paint(renderer, &self.taffy, root_node, self.is_node_hovered(root_node), &mut _pass);
