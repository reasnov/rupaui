...
impl Radio {
    pub fn new(selected: Signal<bool>) -> Self {
        Self { id: generate_id(), selected, style: Style::default() }
    }
}
...
impl Component for Radio {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool) {
        ...
        renderer.draw_rect(layout.location.x, layout.location.y, 20.0, 20.0, [0.3, 0.3, 0.3, 1.0], 10.0);
        if self.selected.get() {
            renderer.draw_rect(layout.location.x + 5.0, layout.location.y + 5.0, 10.0, 10.0, [0.39, 0.45, 1.0, 1.0], 5.0);
        }
    }
...
impl Select {
    pub fn new(options: Vec<String>, selected: Signal<usize>) -> Self {
        Self { id: generate_id(), options, selected_index: selected, style: Style::default() }
    }
}
...
impl Component for Select {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool) {
        ...
        let text = self.options.get(self.selected_index.get()).cloned().unwrap_or_default();
        renderer.draw_text(&text, layout.location.x + 8.0, layout.location.y + 8.0, 14.0, [1.0, 1.0, 1.0, 1.0]);
    }
    fn on_click(&self) {}
}
...
impl Checkbox {
    pub fn new(checked: Signal<bool>) -> Self {
        Self { id: generate_id(), checked, style: Style::default(), accessibility: Accessibility { role: crate::utils::Role::Checkbox, ..Default::default() }, attributes: Attributes::default() }
    }
}
...
impl Switch {
    pub fn new(on: Signal<bool>) -> Self {
        Self { id: generate_id(), on, style: Style::default() }
    }
}
...
impl Component for Switch {
    ...
    fn layout(&self, taffy: &mut TaffyTree<()>, parent: Option<NodeId>) -> NodeId {
        let mut s = self.style.to_taffy();
        s.size = Size { width: length(40.0), height: length(20.0) };
        let node = taffy.new_leaf(s).unwrap();
        if let Some(p) = parent { taffy.add_child(p, node).unwrap(); }
        node
    }
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, _is_group_hovered: bool) {
        let layout = taffy.layout(node).unwrap();
        let bg_color = if self.on.get() { [0.15, 0.68, 0.37, 1.0] } else { [0.3, 0.3, 0.3, 1.0] };
        renderer.draw_rect(layout.location.x, layout.location.y, 40.0, 20.0, bg_color, 10.0);
        let knob_x = if self.on.get() { 22.0 } else { 2.0 };
        renderer.draw_rect(layout.location.x + knob_x, layout.location.y + 2.0, 16.0, 16.0, [1.0, 1.0, 1.0, 1.0], 8.0);
    }
...
impl Input {
    pub fn new(initial_value: impl Into<String>) -> Self {
        ...
        style.padding = crate::utils::spacing::Spacing::all(8.0);

        Self {
            id: generate_id(),
            value: Signal::new(initial_value.into()),
...
impl Component for Input {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, is_group_hovered: bool) {
        ...
        renderer.draw_text(display_text, layout.location.x + 8.0, layout.location.y + 8.0, 14.0, [1.0, 1.0, 1.0, 1.0]);
    }
    fn on_click(&self) {}
}
