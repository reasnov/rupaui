...
impl Viewport {
    pub fn new(content: Box<dyn Component>) -> Self {
        Self {
            id: generate_id(), style: Style::default(), content,
            offset: Signal::new(Vec2::zero()), zoom: Signal::new(1.0),
...
impl Component for Viewport {
    ...
    fn paint(&self, renderer: &mut Renderer, taffy: &TaffyTree<()>, node: NodeId, is_group_hovered: bool, render_pass: &mut wgpu::RenderPass<'_>) {
        ...
        if let Some(content_node) = children.get(0) {
            self.content.paint(renderer, taffy, *content_node, is_group_hovered, render_pass);
        }
