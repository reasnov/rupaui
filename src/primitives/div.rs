...
impl<'a> Div<'a> {
    pub fn new() -> Self {
        ...
        Self { id: generate_id(), logic: DivLogic { children: Children::new() }, view: DivView { core: ViewCore::new(style) } }
    }
    pub fn child(mut self, child: Box<dyn Component + 'a>) -> Self { self.logic.children.add(child); self.view.core.mark_dirty(); self }
}

impl<'a> Stylable for Div<'a> { fn get_style_mut(&self) -> RwLockWriteGuard<'_, Style> { self.view.core.get_style_mut() } }
