...
impl<'a> Flex<'a> {
    ...
    pub fn direction(mut self, dir: FlexDirection) -> Self { self.style.borrow_mut().flex.flex_direction = dir; self.mark_dirty(); self }
    pub fn gap(mut self, val: f32) -> Self { self.style.borrow_mut().flex.gap = Some(val); self.mark_dirty(); self }
...
impl<'a> Component for Flex<'a> {
    ...
    fn on_drag(&self, event: &mut UIEvent, delta: Vec2) { 
        if self.style.borrow().layout.overflow_x == Overflow::Scroll || self.style.borrow().layout.overflow_y == Overflow::Scroll { 
